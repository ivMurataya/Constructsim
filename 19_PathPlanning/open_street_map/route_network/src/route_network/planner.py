...
class Planner():
    ...
    def __init__(self, graph):
        ...
        self.edges = [[] for wp_id in range(len(self.points))]
    ...
    def dijkstra_geo_planner(self, start_segment, goal_segment):
        ...
        closed_list = [False for wp_id in range(len(self.points))]
        ...
        print("Open List IVAN", open_list)
        ...
        while open_list:
            ...
            #Loop Neighbors
            for nei in self.edges[current_node]:
                # Check if the neighbor has already been visited
                if  closed_list[nei.end] == True:
                    continue
                
                # calculate g_cost of neighbour considering it is reached through current_node
                g_cost = current_travel_cost + nei.step_cost 
                
                # Check if the neighbor is in open_list
                in_open_list = False
                for idx, element in enumerate(open_list):
                    if element[1] == nei.end:
                        in_open_list = True
                        previous_cost = element[0]
                        break
                # CASE 1: neighbor already in open_list
                if in_open_list:
                    if g_cost <   previous_cost:
                        # nei.step_cost = g_cost
                        parents[nei.end] =  [current_node,nei]
                        open_list[idx] = [g_cost ,nei.end ]
                    ...
                    open_list.append([g_cost, nei.end ])
                    

        ##### YOUR CODE ENDS HERE #####
        print("1 Closed List", closed_list)
        print("Value", closed_list[1342], "leng", len(closed_list))
        print("Edges lwngt", len(self.edges))
