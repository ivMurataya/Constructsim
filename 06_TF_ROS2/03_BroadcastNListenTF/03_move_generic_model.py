...
class CamBotMove(Node):
    ...
    def __init__(self, timer_period=0.05, model_name="cam_bot", init_dest_frame="carrot", trans_speed=1.0, rot_speed=0.1):
        ...
        # For the TF listener
        """
        The lines of code above start a TF listener object. Note that you also require a Buffer object.
        """
    ...
    def get_model_pose_from_tf(self, origin_frame="world", dest_frame="camera_bot_base_link"):
        ...
        try:
            now = rclpy.time.Time()
            #This line here checks the buffer and looks for the transforms from origin_frame to dest_frame at the time now:
        ...
        """Finally, use the information from the transform to create a Pose() 
        object to move the Cam_bot robot to that precise location. 
        This places the robot in the same frame as the one you asked the 
        transform to through a topic named /destination_frame."""
        pose = Pose()
