...
class SubscriberClass(Node):
    ...
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(Odometry, 'odom', self.odom_callback, 10, callback_group=self.mutuallyexclusive_group_1)
        self.scan_sub = self.create_subscription(LaserScan, 'scan', self.scan_callback, QoSProfile(depth=10, reliability=ReliabilityPolicy.BEST_EFFORT), callback_group=self.mutuallyexclusive_group_2)
        self.timer = self.create_timer(0.5, self.timer_callback, callback_group=self.mutuallyexclusive_group_3)
