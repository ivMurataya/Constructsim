...
def main(args=None):
    ...
    start_stop_service_node = MoveServiceServer(cmd_vel_topic="/cmd_vel", timer_period = 0.5)
...
"""
The Service Server Callback SetBool_callback from the service server called /start_turn.
WE SET A FLAG TO ONLY PUBLISH WHEN IT STARTS AND WHEN SERVICE IS CALLED.
The Timer Callback timer_callback.
A new service called /calculations, of message type std_srvs/SetBool, declared in a different Node class, that you would call CalculationsServiceServer.
This CalculationsServer Node has to be subscribed to the laser data and retrieve the front laser values (self.laser_forward = msg.ranges[359]).
This CalculationsServer Node's service server should retrieve the latest laser front data, wait five seconds and then retrieve the laser data again. 
Then calculate the difference and send a response depending on if the delta is positive or negative. And it should also publish in the cmd_vel to go backward or forwards based on the sign of delta
"""

"""
ros2 run multithreading_ros2_examples calc_node
ros2 service call /start_turn std_srvs/SetBool "data: true"
ros2 service call /calculations std_srvs/SetBool "data: false"
"""
