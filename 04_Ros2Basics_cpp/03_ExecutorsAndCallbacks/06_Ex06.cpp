...
This is why only one thread gets created, and only one Callback can be performed. This is because the Multi-Threaded Executor creates one thread per Node if you do not specify otherwise. So you only have one thread for all Callbacks in this Node; therefore, it cannot execute Callbacks in parallel.

The timer1_callback is executed because it has been instantiated first in the constructor.
*/
