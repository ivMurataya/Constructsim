...
def normalize(inputList):
    ...
    # Step 1: Compute the sum of the likelihood values
    total = float(sum(inputList))

    # Step 2: Normalize each value by dividing it by the total
    normalized_likelihood = [float(n) / total for n in inputList]
...
def correct_step(likelihood, belief):
    ...
    for i in range(len(likelihood)):
        unnormalized.append(likelihood[i] * belief[i])
    normalized_belief = normalize(unnormalized)
