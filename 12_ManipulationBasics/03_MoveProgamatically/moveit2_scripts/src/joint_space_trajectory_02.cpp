...
// This program takes two positions, a joint position and a goal position, plan and then execute them
...
public:
  ...
  void execute_trajectory_plan() {
    ...
    while (moving_ < 6){
        if (pose1_){
            RCLCPP_INFO(LOGGER, "1 Preparing Joint Value Trajectory...");
            setup_joint_value_target(+0.0000, -2.3562, +1.5708, -1.5708, -1.5708, +0.0000);
    // plan and execute the trajectory
    RCLCPP_INFO(LOGGER, " 1 Planning Joint Value Trajectory...");
    plan_trajectory_kinematics();

    RCLCPP_INFO(LOGGER, "1 Executing Joint Value Trajectory...");
    execute_trajectory_kinematics();

    RCLCPP_INFO(LOGGER, " 1 Joint Space Trajectory Planning Complete");
    pose1_ = false;
  }
  else{
  RCLCPP_INFO(LOGGER, "2 Preparing GOAL Value Trajectory...");
  setup_goal_pose_target(+0.343, +0.132, +0.284, -1.000, +0.000, +0.000,
                           +0.000);
    // plan and execute the trajectory
    RCLCPP_INFO(LOGGER, "2 Planning GOAL Value Trajectory...");
    plan_trajectory_kinematics();

    RCLCPP_INFO(LOGGER, "2 Executing GOAL Value Trajectory...");
    execute_trajectory_kinematics();

    RCLCPP_INFO(LOGGER, " 2 GOAL Space Trajectory Planning Complete");
    pose1_ = true;
  }
  moving_ ++;
    }
