...
"""
    ...
    The only difference is that now the node plant_detector_node is managed not directly by the rclpy.spin, but by the executor object.
    This MultiThreadedExecutor allows you to specify HOW MANY THREADS you need.

    Typically, you need ONE THREAD PER CALLBACK to ensure that both can be executed simultaneously.
    
    The rest of the structure remains essentially the same:
