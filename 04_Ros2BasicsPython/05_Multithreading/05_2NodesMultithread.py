...
In this example, you add a new node named green_detector_node to the executor.

This will be a node used to double check plant detections, specifically to verify if they are green, which is associated with plants on Earth.
This node will have a new service callback named detect_green_callback.

This means that you are initializing an additional THREAD in a separate script.

However, if you use the same executor, you use the TWO threads you have more efficiently.
It also gives direct access to the PlantDetectorNode and all its data, especially the image data.
This structure makes your system more efficient in terms of threads, using one less.



We create a new Node class named GreenDetectorNode.
Inside it the main element is detect_green_detector(cv_image) method, that uses OpenCV to generate a mask for ONLY the green colour. 
It will output an image with GREEN color masking when the service /green_detector is called.
...
rm -rf ~/.ros/tracing/green_detector/
ros2 launch basics_ros2_multithreading  green_detector_node.launch.py


Now you have 2 nodes on the same program, therefore you have 2 services actives:
